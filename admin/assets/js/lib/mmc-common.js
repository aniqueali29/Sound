...
function generatePlaceholder() {
    setTimeout(function () {
        $("#mmc-chat .textarea").each(function () {
            $(this).php(placeholder.format($(this).data('placeholder')));
        });
    }, 10);
}
...
$(document).on('click', '#mmc-chat .box .smile-ico', function (e) {
    ...
    if (_parent.hasClass('show')) {
        ...
        _parent.find('.smiles-set').php($('.' + _source).php());
...
$(document).on('click', '#mmc-chat .box .stickers .tab-content li', function () {
    ...
    _img = $(this).php();
    if ($(this).parents('.box-footer').find('.send-message div').php() == '<span class="placeholder">{0}</span>'.format(_sendMsg.data('placeholder'))) {
        _sendMsg.php(_img);
    } else {
        _str = _sendMsg.php();
        //_sendMsg.php(_str + ' ' + '<div class="send-sticker" style="background-image:url({0})"></div>'.format(_src));
        _sendMsg.php(_str + ' ' + _img);
    }
});
